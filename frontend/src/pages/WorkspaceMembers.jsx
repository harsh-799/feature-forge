...
import { getWorkspaceMembers, inviteWorkspaceMember, removeWorkspaceMember } from '../api/workspaceApi'
                            ...
                            <tr key={idx} style={{ borderBottom: idx === filteredMembers.length - 1 ? 'none' : '1px solid var(--border-color)' }}>
                              ...
                              <td className="member-badge-cell">
                                <span style={{
                                  ...
                                  backgroundColor: m.role === 'ADMIN' ? 'rgba(38,37,33,0.06)' : m.role === 'QA' ? 'rgba(59,130,246,0.08)' : 'rgba(249,115,22,0.08)',
                                  color: m.role === 'ADMIN' ? 'var(--text-heading)' : m.role === 'QA' ? '#3B82F6' : 'var(--accent)'
                                }}>
                                  {m.role}
                                </span>
                              </td>
                              <td className="member-badge-cell">
                                {m.status === 'PENDING' ? (
                                  <span style={{ fontSize: '10px', fontWeight: 700, color: '#B45309', backgroundColor: '#FEF3C7', padding: '3px 7px', borderRadius: '4px' }}>PENDING</span>
                                ) : (
                                  <span style={{ fontSize: '10px', fontWeight: 700, color: '#2F9254', backgroundColor: '#EBF7EE', padding: '3px 7px', borderRadius: '4px' }}>ACTIVE</span>
                                )}
                              </td>
